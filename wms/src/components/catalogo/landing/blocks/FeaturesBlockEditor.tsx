...
const iconOptions = ['truck', 'shield', 'refresh', 'star', 'heart', 'gift', 'check', 'clock', 'phone', 'mail'];
...
export default function FeaturesBlockEditor({ content, onUpdate }: FeaturesBlockEditorProps) {
  ...
  const addItem = () => {
    if (items.length < 6) {
      onUpdate({ items: [...items, { icon: 'check', title: '', description: '' }] });
    }
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <label className="text-xs text-gray-500">Caracteristicas ({items.length}/6)</label>
        <button
          type="button"
          onClick={addItem}
