...
import { createShipment as createOlvaShipment, isConfigured } from '@/lib/logistics/olva';
...
export async function POST(request: NextRequest, { params }: Props) {
  try {
    if (!isConfigured()) {
      return apiError('Olva API no configurada. Obtener credenciales en olva.com.pe/empresas o llamar al 01-612-1212', 400);
    }
    ...
    const result = await createOlvaShipment({
    ...
    if (!result.success) {
      return apiError(result.error || 'Error al crear guia Olva', 500);
    }
    ...
    await prisma.shipmentEvent.create({
      data: { shipmentId: shipment.id, status: 'label_created', description: `Guia Olva creada: ${result.guideNumber}` },
    });
  ...
  } catch (error) { return handleApiError(error, 'olva-create'); }
}
